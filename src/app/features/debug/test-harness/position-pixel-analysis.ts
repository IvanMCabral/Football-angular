...
export function strictPositionPixelLine(position: string | null | undefined): PositionPixelVisualLine | null {
  ...
  if (normalized === 'DEF' || normalized.includes('BACK')) return 'DEF';
  if (normalized === 'ATT' || normalized.includes('STRIKER') || normalized.includes('FORWARD')) return 'ATT';
  if (normalized === 'MID' || normalized.includes('WINGER')) return 'MID';
  return null;
}
...
export function positionPixelCoachRead(row: PositionPixelMatrixSummary): string {
  ...
  const defensiveRisk = positionPixelDefensiveRiskScore(row);
  const defensiveGain = positionPixelDefensiveGainScore(row);
  const coverageBreakdown = positionPixelChannelBreakdown(row).coverage;
  const sourceLine = strictPositionPixelLine(row.playerPosition) ?? positionPixelVisualLine(row.fromYPercent);
  const contextualCoverage = positionPixelUsesContextualCoverage(row, sourceLine, coverageBreakdown);
  ...
  if (positionPixelDistance(row) <= 1.25) {
  ...
  if (movedUp && defensiveRisk >= 0.9 && attackGain < 0.7) {
    const targetLine = positionPixelVisualLine(row.targetYPercent);
    if (sourceLine === 'DEF' && targetLine === 'MID' && positionPixelDistance(row) > 10) {
